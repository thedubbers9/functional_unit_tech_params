...
module cordic_flopped (
    ...
    logic [BITWIDTH-1:0] cordic_x, cordic_y, cordic_z;

    logic [BITWIDTH-1:0] y0;
    assign y0 = 16'b0;

    cordic_comb #(.vectoring(0), .width(16)) iDUT (  
        .x0(multiplied_xor_result_flopped_a),
        .y0(y0),
        .z0(multiplied_xor_result_flopped_b), 
        .x(cordic_x),
        .y(cordic_y),
        .z(cordic_z)
    ); 

    assign sum_unflopped[0] = cordic_x + cordic_y;

    genvar j;
    generate
        // Flop the result through the pipeline
