...
module fp_addsub_pipe5 #(
  ...
  parameter int SIG = 24
    ...
    logic [E+S-1:0] a_ieee_reg, b_ieee_reg;
    logic sub_reg;
    logic [2:0] roundingMode_reg;
    logic detectTininess_reg;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            sub_reg <= 0;
            roundingMode_reg <= 0;
            detectTininess_reg <= 0;
        end else begin
            a_ieee_reg <= a_ieee;
            b_ieee_reg <= b_ieee;
            sub_reg <= sub;
            roundingMode_reg <= roundingMode;
            detectTininess_reg <= detectTininess;
        end
    end

    // Stage 2: fNToRecFN conversion
    logic [E+S:0] a_rec_reg, b_rec_reg;
    logic [E+S:0] a_rec_reg_t, b_rec_reg_t;
    fNToRecFN #(.expWidth(E), .sigWidth(S)) i_f2rec_a (.in(a_ieee), .out(a_rec_reg_t));
    fNToRecFN #(.expWidth(E), .sigWidth(S)) i_f2rec_b (.in(b_ieee), .out(b_rec_reg_t));
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            a_rec_reg <= 0;
            b_rec_reg <= 0;
        end else begin
            a_rec_reg <= a_rec_reg_t;
            b_rec_reg <= b_rec_reg_t;
        end
    end

    // Stage 3: addRecFN
    logic [E+S:0] z_rec_reg;
    logic [4:0] exceptionFlags_reg;
    logic [E+S:0] z_rec_reg_t;
    logic [4:0] exceptionFlags_reg_t;
    addRecFN #(.expWidth(E), .sigWidth(S)) i_addsub (
                .control        (detectTininess_reg),
                .subOp          (sub_reg),
                .a              (a_rec_reg),
                .b              (b_rec_reg),
                .roundingMode   (roundingMode_reg),
                .out            (z_rec_reg_t),
                .exceptionFlags (exceptionFlags_reg_t)
            );
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            z_rec_reg <= 0;
            ...
            z_rec_reg <= z_rec_reg_t;
            exceptionFlags_reg <= exceptionFlags_reg_t;
        end
    end

    // Stage 4: recFNToFN conversion
    logic [E+S-1:0] z_ieee_reg, z_ieee_reg_t;
    recFNToFN #(.expWidth(E), .sigWidth(S)) i_rec2f (.in(z_rec_reg), .out(z_ieee_reg_t));
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            z_ieee_reg <= 0;
        end else begin
            z_ieee_reg <= z_ieee_reg_t | (z_rec_reg_t | b_rec_reg_t | a_rec_reg_t | roundingMode | detectTininess); // for timing and avoiding optimisation out
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            z_ieee <= z_ieee_reg;
            exceptionFlags <= exceptionFlags_reg;
        end
    end
endmodule