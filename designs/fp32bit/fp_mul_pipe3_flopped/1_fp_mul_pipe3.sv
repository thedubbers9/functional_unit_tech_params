...
module FP_MUL_PIPE3 #(
    ...
    // Stage 2: Multiply, normalize, prepare rounding
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            stage2_reg <= '0;
        end else begin
    ...
    always_comb begin
        ...
        // Predefined values
        stage2_next.bias = {(EXPONENT_WIDTH - 1) {1'b1}};
        stage2_next.quiet_nan = {1'b1, {EXPONENT_WIDTH{1'b1}}, 1'b1, {(MANTISSA_WIDTH - 1) {((EXPONENT_WIDTH == 4 && MANTISSA_WIDTH == 3) ? 1'b1 : 1'b0)}}};
        // Unpack
        
        {a_sign, a_exponent, a_mantissa} = stage1_reg.a;
        {b_sign, b_exponent, b_mantissa} = stage1_reg.b;
        a_implicit_leading_bit = !(a_exponent == 0);
        b_implicit_leading_bit = !(b_exponent == 0);
        // Multiply
        stage2_next.out_sign = a_sign ^ b_sign;
        stage2_next.a_mul_b_mantissa = {a_implicit_leading_bit, a_mantissa} * {b_implicit_leading_bit, b_mantissa};
        stage2_next.a_mul_b_exponent = a_exponent + b_exponent - stage2_next.bias;
        stage2_next.leading_one_is_MSB = stage2_next.a_mul_b_mantissa[(MANTISSA_WIDTH+1)*2-1];
    end

    // Stage 3: Round, pack result, set flags
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            stage3_reg <= '0;
        end else begin
    ...
    result_rounder #(EXPONENT_WIDTH, MANTISSA_WIDTH, ROUND_TO_NEAREST_TIES_TO_EVEN, MANTISSA_WIDTH + 1) result_rounder_block (
        .non_rounded_exponent(stage3_next.non_rounded_exponent),
        .non_rounded_mantissa(stage3_next.non_rounded_mantissa),
        .rounding_bits(stage3_next.additional_mantissa_bits),
    ...
    always_comb begin
        stage3_next.underflow_flag = 1'b0;
        stage3_next.overflow_flag = 1'b0;
        stage3_next.invalid_operation_flag = 1'b0;
        stage3_next.out_sign = stage2_reg.out_sign;
        // Special value handling
        if (stage2_reg.is_signaling_nan_a || stage2_reg.is_signaling_nan_b || stage2_reg.is_quiet_nan_a || stage2_reg.is_quiet_nan_b) begin
            stage3_next.out = stage2_reg.quiet_nan;
            if ((stage2_reg.is_signaling_nan_a || stage2_reg.is_signaling_nan_b) || ((stage2_reg.is_quiet_nan_a && (!stage2_reg.is_signaling_nan_b && !stage2_reg.is_quiet_nan_b)) || (stage2_reg.is_quiet_nan_b && (!stage2_reg.is_signaling_nan_a && !stage2_reg.is_quiet_nan_a)))) begin
                stage3_next.invalid_operation_flag = 1'b1;
            end
        end else if ((stage2_reg.is_a_zero && stage2_reg.is_b_infinite) || (stage2_reg.is_b_zero && stage2_reg.is_a_infinite)) begin
            stage3_next.out = stage2_reg.quiet_nan;
            stage3_next.invalid_operation_flag = 1'b1;
        end else begin
            // Normal/overflow/underflow
            if (stage2_reg.a_mul_b_exponent < 0 || (stage2_reg.a_mul_b_exponent[EXPONENT_WIDTH+1-1:0] == 0 && stage2_reg.leading_one_is_MSB == 1'b0)) begin
                stage3_next.out_exponent = 0;
                stage3_next.out_mantissa = 0;
                stage3_next.underflow_flag = 1'b1;
            end else if (stage2_reg.a_mul_b_exponent[EXPONENT_WIDTH+1-1:0] >= {EXPONENT_WIDTH{1'b1}} || (stage2_reg.a_mul_b_exponent[EXPONENT_WIDTH-1:0] == ({EXPONENT_WIDTH{1'b1}} - 1) && stage2_reg.leading_one_is_MSB)) begin
                stage3_next.out_exponent = {EXPONENT_WIDTH{1'b1}};
                stage3_next.out_mantissa = {MANTISSA_WIDTH{1'b0}};
                stage3_next.overflow_flag = 1'b1;
            end else begin
                stage3_next.non_rounded_exponent = (stage2_reg.is_a_zero || stage2_reg.is_b_zero) ? 0 : stage2_reg.a_mul_b_exponent[EXPONENT_WIDTH-1:0] + (stage2_reg.leading_one_is_MSB ? 1 : 0);
                stage3_next.non_rounded_mantissa = stage2_reg.leading_one_is_MSB ? stage2_reg.a_mul_b_mantissa[2*MANTISSA_WIDTH:MANTISSA_WIDTH+1] : stage2_reg.a_mul_b_mantissa[2*MANTISSA_WIDTH-1:MANTISSA_WIDTH];
                stage3_next.additional_mantissa_bits = stage2_reg.leading_one_is_MSB ? stage2_reg.a_mul_b_mantissa[MANTISSA_WIDTH:0] : stage2_reg.a_mul_b_mantissa[MANTISSA_WIDTH-1:0] << 1;
                
                stage3_next.out_mantissa = rounded_mantissa;
                stage3_next.out_exponent = rounded_exponent;
                stage3_next.overflow_flag = rounded_overflow_flag;
            end
            stage3_next.out = {stage3_next.out_sign, stage3_next.out_exponent, stage3_next.out_mantissa};
        end
    end

    // Output assignments
    always_comb begin
        out = stage3_reg.out | (stage2_next.a_mul_b_mantissa | stage2_next.a_mul_b_exponent | stage1_next | stage2_next); // for timing purposes to account for other pipeline stages, and prevent optimisation out
        underflow_flag = stage3_reg.underflow_flag;
        overflow_flag = stage3_reg.overflow_flag;
        invalid_operation_flag = stage3_reg.invalid_operation_flag;
    end
endmodule
